...
PMULTIPLE_FMs =["Tarboton", "Dinf", "Quinn", "FairfieldLeymarieD8", "FairfieldLeymarieD4", "OCallaghanD8", "OCallaghanD4", "Freeman", "Holmgren"]
...
class FlowDirAccPf(Component):
    ...
    FlowAccumulatorPf stores as ModelGrid fields:
        ...
        -  Length to adjacent nodes, squared (calcualted in advance to save time  
            during subsequent updates *'SQUARED_length_adjacent'                                       
            
        -  TODO Node array of links carrying flow is currently not supported in FlowAccumulatorPf, 
            use FlowAccumulator instead.
            *'flow__link_to_receiver_node'*. is returned as an np array containing 
            only -100 values          
    ...
    **Additional References**
    ...
    def __init__(
        ...
        # Currently, flow links are not updated in PF router: 
        self.grid.at_node["flow__link_to_receiver_node"] = -100*np.int64(np.ones_like(self.grid.at_node["topographic__elevation"]))
    ...
    def _create_properties(self):
        ...
        # self._props_Pf = rd.FlowProportions(dem=self._deprFreeDEM,
        #                                        method=self._flow_metric,exponent=self._exponent)
        # self._fa = rd.FlowAccumFromProps(props=self._props_Pf)
        ...
        # if self._seperate_Hill_Flow:
        #     self._props_hill_Pf = rd.FlowProportions(dem=self._deprFreeDEM,
        #                                        method=self._hill_flow_metric,exponent=self._hill_exponent)
    ...
    def calculateFlowDir_Acc(self, hillFlow=False, updateDepressions=True):
        ...
        if flowMetric == 'D8':
            ...
            # Convert flow proportions to landlap structure        
    ...
    def FA_DA_D8(self, hillFlow=False):
        ...
        _FD_D8(receivers,distance_receiver,steepest_slope,np.array(el),el_ori ,
                                          dist,ngb,el_d,
                                          r,c,dx,sq2)
        ...
        if do_FA:        
            if any(self.grid.at_node["water__unit_flux_in"]!=1):
                ...
                dis = np.zeros(self.grid.number_of_nodes, dtype=int) + self._node_cell_area
    ...
    def run_one_step(self):
        if self._suppress_out:
            with suppress_stdout_stderr():
                sys.stdout = open(os.devnull, "w")
                sys.stderr = open(os.devnull, "w")
                self.calculateFlowDir_Acc(hillFlow=False)
                if self._seperate_Hill_Flow:
                    if self._update_HillFlow_instanteneous:
                        self.calculateFlowDir_Acc(hillFlow=True, updateDepressions=False)

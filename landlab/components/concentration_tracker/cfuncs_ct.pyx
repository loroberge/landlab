...
cpdef _landslide_runout(
    ...
    np.ndarray[DTYPE_FLOAT_t, ndim=1] CV_in,
    np.ndarray[DTYPE_FLOAT_t, ndim=1] L_Hill,
    np.ndarray[DTYPE_FLOAT_t, ndim=1] Qs_out,
    np.ndarray[DTYPE_FLOAT_t, ndim=1] CV_out,
    np.ndarray[DTYPE_FLOAT_t, ndim=1] dH_Hill,
    np.ndarray[DTYPE_FLOAT_t, ndim=1] dCV_Hill,
    ...
    This version includes a mass balance calculation of sediment property concentration.
    """
    # define internal variables
    cdef int donor, rcvr, r
    cdef double accum, proportion, dH, dCV
    ...
    for donor in stack_rev_sel:
        ...
        # Calculate change in CV from fraction of total volume being deposited
        # (C = sed property concentration, V = sed volume, so C*V = sed property mass)
        dCV = 0.0
        if Qs_in[donor] > 0:
            dCV = CV_in[donor] * (dH * dx * dx * (1 - phi)) / Qs_in[donor]

        # Remove CV that is being deposited
        CV_in[donor] -= dCV
        # Record deposited CV (to use later in _landslide_runout() method)
        dCV_Hill[donor] += dCV
        # All remaining CV will be apportioned between receivers in loop below
        CV_out[donor] += CV_in[donor]
        ...
        for r in range(receivers.shape[1]):
            ...
            if proportion > 0. and donor != rcvr:
                ...
                CV_in[rcvr] += CV_out[donor] * proportion
                CV_in[donor] -= CV_out[donor] * proportion
